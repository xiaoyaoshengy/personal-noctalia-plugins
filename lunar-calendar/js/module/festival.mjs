import { getDateString } from "./tool.mjs";
import { getSolarByTimestamp } from "./solar.mjs";
import { getLunarMonthDays } from "./lunar.mjs";
import { getTermDate } from "./term.mjs";
import { getGanZhiDay } from "./ganzhi.mjs";
...
export function getFestivalsBySolar(sYear, sMonth, sDay) {
    ...
    return [
        ...
        .map(function (list = []) {
            return list
                .filter((item) => dateFull >= item.found)
                .map((item) => item.name);
        })
        .reduce((pre, item) => {
...
export function getTermFestivalsBySolar(sYear, sMonth, sDay) {
    ...
    (function () {
        ...
        count = 0;
        for (
            let time = qiufen_time;
            time <= qiufen_time + dayTime * 20;
            time += dayTime
        ) {
            let solar = getSolarByTimestamp(time);
            let ganzhi = getGanZhiDay(solar["sYear"], solar["sMonth"], solar["sDay"]);
    ...
    (function () {
        ...
        let last_dongzhi_time = new Date(
            sYear - 1,
            11,
            last_termDate[23],
        ).getTime();
        ...
        count = 0;
        for (
            let time = dongzhi_time;
            time <= dongzhi_time + 8 * 9 * dayTime;
            time += 9 * dayTime
        ) {
            let solar = getSolarByTimestamp(time);
            if (
...
export function getFestivalsByLunar(lYear, lMonth, lDay) {
    ...
    if (lMonth == 12 && lDay == getLunarMonthDays(lYear, 12)) {
        ...
        festivals = festivals.concat(
            [lFestival[dateKey], lFestival2[dateKey]]
                .map(function (list = []) {
                    return list
                        .filter((item) => dateFull >= item.found)
                        .map((item) => item.name);
